...
from .settings import args, FILL_VAL
...
class Regularizer(abc.ABC):
    def __init__(self, model, parallel_model, dataloaders, task, output_dir, src_dir=None, prev_task=None):
        self.model = model
    ...
    def load_reg_params(self, noload_prefix=()):
        #if self.prev_task:
        assert self.src_dir
        model_dir = self.src_dir
        reg_params_path = os.path.join(model_dir, "reg_params.pkl")
        with open(reg_params_path, 'rb') as f:
            unamed_loaded_reg_params = pkl.load(f)
            # added:
            reg_params = {}
            for name, param in self.model.named_parameters():
                if startswith_any(noload_prefix, name):
                    ...
                    flg = False
                    for k,v in unamed_loaded_reg_params.items():
                        loaded_param_name = v['name']
                        if name == loaded_param_name:
                            reg_params[param] = v
                            flg = True
                            break
                    if not flg:
                        raise ValueError('no param in the weight pkl')
        if 'data_count' not in reg_params:
            reg_params['data_count'] = 0
        reg_params['lambda'] = args.reg_lambda
        self.model.reg_params = reg_params
...
class EWC(Regularizer):
    def task_start_do(self,freeze_layers=[]):
        #self.load_reg_params()
        task_start_do(self.model, freeze_layers)
    def task_end_do(self, explainer, expl_importance):
        updater = Omega_update(self.model.parameters(), lr=0.0001, momentum=0.9)
        compute_importance(self.model, self.parallel_model, updater, self.dataloaders, loss_type="ewc",
                           explainer=explainer, expl_importance=expl_importance)
        accumulate_reg_params(self.model)
        self.save_reg_params()
...
def initialize_reg_params(model,freeze_layers=[], init_value=0):
    """initialize an omega for each parameter to zero or one"""
    reg_params={}
    for name, param in model.named_parameters():
        if not startswith_any(freeze_layers, name):
        #if not name in freeze_layers:
            # print('initializing param',name)
            omega=torch.FloatTensor(param.size()).zero_()
            if init_value != 0:
                omega.fill_(init_value)
            ...
            #initialize the initial value to that before starting training
            reg_param['init_val'] = init_val
            # update: param fixed to param name
            reg_params[param]=reg_param
    if 'data_count' not in reg_params:
        reg_params['data_count'] = 0
    reg_params['lambda'] = args.reg_lambda
...
class Weight_Regularized_AdamW(Optimizer):
    ...
    def step(self, reg_params, closure=None):
        ...
        reg_lambda = args.reg_lambda #reg_params.get('lambda')
...
def compute_importance(model, parallel_model, updater, dataloaders, loss_type,
                       explainer, expl_importance):
    ...
    if loss_type == "l2":
        ...
        CELoss = CrossEntropyLoss(ignore_index=FILL_VAL, reduction='mean')
        loss_fct = DataParallelCriterion(CELoss, args.device_ids)

    batch_size = None
    # Iterate over data.
    for dataloader in dataloaders:
        for step, inputs in enumerate(tqdm(dataloader,desc='computing ewc importance')):
            ...
            for k, v in inputs.items():
                inputs[k] = v.to(args.device_ids[0])
            ...
            if loss_type != "ewc":
                ...
                if not expl_importance:
                    model_ret = model(**inputs)
                    batch_size = inputs['input_ids'].size(0)
                    logits = model_ret[1]
                    targets = model_ret[0]
                    targets.backward()
                    updater.step(model.reg_params, batch_size)
                else:
                    def update_func():
                        updater.step(model.reg_params, batch_size=1)
                    ...
                    # logger.info('reg loss: {}'.format(reg_loss))
