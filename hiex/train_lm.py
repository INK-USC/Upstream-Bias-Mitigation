...
def do_train_lm(model, lm_dir, lm_epochs, train_iter, dev_iter, save_every=500, dev_every=500):
    opt = O.Adam(filter(lambda x: x.requires_grad, model.parameters()), lr=0.0005)
    ...
    for epoch in range(lm_epochs):
        if all_break:
            break
        train_loss = 0
        for batch_idx, rb_batch in enumerate(train_iter):
            if type(rb_batch) is dict:
                batch = convert_roberta_batch(rb_batch)
            else:
                batch = rb_batch
            ...
            if iterations % dev_every == 0:
                ...
                for dev_batch_idx, rb_dev_batch in enumerate(dev_iter):
                    if type(rb_dev_batch) is dict:
                        dev_batch = convert_roberta_batch(rb_dev_batch)
                    else:
                        dev_batch = rb_dev_batch
