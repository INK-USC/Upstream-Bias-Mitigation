...
class _SamplingAndOcclusionBaseAlgo:
    ...
    def do_attribution(self, input_ids, input_mask, token_type_ids, region, label=None):
...
class _SamplingAndOcclusionAlgo(_SamplingAndOcclusionBaseAlgo):
    def __init__(self, model, tokenizer, lm_model, output_path, configs):
        ...
        if configs.noise_lm:
            self.lm_model.noise_lm = True

    def occlude_input_with_masks_and_run(self, inp_bak, inp_mask, token_type_ids, x_regions, nb_regions, label,
                                         importance_target,
        ...
        for sample_i in range(self.sample_num + extra):
            ...
            for i in range(len(filled_inp)):
                if self.configs.keep_other_nw or not (i in additional_mask and not x_region[0] <= i <= x_region[1]):
                    ...
                    # pass # TODO: test performance of omission
                    filled_inp_.append(self.pad_token)
                    mask_inp_.append(0)
            ...
            for i in range(len(filled_inp)):
                if not x_region[0] <= i <= x_region[1] and (i not in additional_mask or self.configs.keep_other_nw):
                    ...
                    if self.configs.keep_other_nw:
                        #pass # TODO: test performance of omission
                        filled_ex.append(self.pad_token)
                        mask_ex.append(0)
        ...
        if self.gpu >= 0:
            ...
            token_type_ids = token_type_ids.to(self.gpu) if token_type_ids is not None else None

        inp_enb_mask = inp_enb_mask.expand(inp_enb.size(0), -1)
        token_type_ids = token_type_ids.expand(inp_enb.size(0), -1) if token_type_ids is not None else None

        if importance_target == 'output':
            logits_enb = self.model(input_ids=inp_enb,
                                    token_type_ids=token_type_ids[:, :inp_enb.size(1)] if token_type_ids is not None else None,
                                    attention_mask=inp_enb_mask)
            logits_ex = self.model(input_ids=inp_ex,
                                   token_type_ids=token_type_ids[:, :inp_ex.size(1)] if token_type_ids is not None else None,
            ...
            if self.configs.pos_contrib:
                contrib_score = F.relu(contrib_score)
        elif importance_target == 'hidden':
            _, layer_outputs_enb, __ = self.model(input_ids=inp_enb,
                                    token_type_ids=token_type_ids[:, :inp_enb.size(1)] if token_type_ids is not None else None,
                                    attention_mask=inp_enb_mask)[:3]
            _, layer_outputs_ex, __ = self.model(input_ids=inp_ex,
                                   token_type_ids=token_type_ids[:, :inp_ex.size(1)] if token_type_ids is not None else None,
                                   attention_mask=inp_ex_mask)[:3]
            # get the representation of the cls token
            hidden_enb = layer_outputs_enb[-1][:,0,:]
            hidden_ex = layer_outputs_ex[-1][:,0,:]
            contrib_score = ((hidden_enb - hidden_ex) ** 2).sum(-1)
    ...
    def do_oc_attribution_batched(self, input_ids, input_mask, token_type_ids, regions):


        raise NotImplementedError

    def do_attribution(self, input_ids, input_mask, token_type_ids, region, label=None, return_variable=False,
                       additional_mask=[], importance_target='output'):
        ...
        score = self.occlude_input_with_masks_and_run(inp_flatten, inp_mask_flatten, token_type_ids, [region],
                                                      mask_regions, label,
    ...
    def do_attribution_pad_variant(self, input_ids, input_mask, token_type_ids, region, label=None, return_variable=False,
                                   additional_mask=[], importance_target='output'):
        """
        ...
        if self.gpu >= 0:
            ...
            token_type_ids = token_type_ids.to(self.gpu) if token_type_ids is not None else None
        logits_enb = self.model(input_ids=inp_enb, token_type_ids=token_type_ids[:, :inp_enb.size(1)] if token_type_ids is not None else None, attention_mask=inp_mask_enb)
        logits_ex = self.model(input_ids=inp_ex, token_type_ids=token_type_ids[:, :inp_ex.size(1)] if token_type_ids is not None else None, attention_mask=inp_mask_ex)
    ...
    def do_hierarchical_explanation(self, input_ids, input_mask, token_type_ids, label_ids):
        logits_pred = self.model(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=input_mask)
        logits_pred = logits_pred[:,1] - logits_pred[:,0]

        inp = input_ids.view(-1).cpu().numpy()
        lists = self.agglomerate((input_ids, input_mask, token_type_ids), percentile_include=90, method='cd',
