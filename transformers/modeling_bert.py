...
from .modeling_ubm import UBMFrameworkAbc
...
class BertForSequenceClassification(BertPreTrainedModel,UBMFrameworkAbc):
    def __init__(self, config):
        super().__init__(config)
        UBMFrameworkAbc.__init__(self,config, classifier_head_cls=
                                 lambda x: nn.Linear(x.hidden_size, x.num_labels))
    ...
    Examples::
        ...
        if labels is not None:
            ...
            if task_id in self.mtl_task_id_to_reg_args and self.mtl_task_id_to_reg_args[task_id].adv_debias:
                ...
                attr_logits = self.mtl_adv_classifiers[task_id-1](pooled_output[:,0,:], rev_grad=True)
                attr_probs = torch.nn.Softmax(-1)(attr_logits)
