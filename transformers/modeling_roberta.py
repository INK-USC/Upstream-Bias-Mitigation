...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.reg_args = None
        self.adv_model = None
        # self.adv_objective = None
        # self.adv_strength = None
        # self.label_stats = None
        # self.adv_grad_rev_strength = None
    ...
    def add_mtl_classifier(self, num_label):
        ...
        self.mtl_classifiers.append(RobertaClassificationHead(tmp_config))
    ...
    def set_class_weight(self, v):
        print('class weight set to', v)
        self.class_weight = v
    ...
    Examples::
        ...
        if task_id == 0 or task_id is None:
            ...
            # if self.use_head is not None:
            #     logits = self.mtl_classifiers[self.use_head - 1](sequence_output)
            # else:
            #     logits = self.mtl_classifiers[head_id-1](sequence_output)
            logits = self.mtl_classifiers[task_id - 1](sequence_output)
        ...
        if labels is not None:
            ...
            if no_main_objective:
                loss = loss * 0

            # main task adv debiasing, if it is the main task
            if self.adv_debias and (task_id == 0 or task_id is None):
            ...
            # multi-task adv debiasing

            if task_id in self.mtl_task_id_to_reg_args and self.mtl_task_id_to_reg_args[task_id].adv_debias:
                reg_args = self.mtl_task_id_to_reg_args[task_id]
                attr_logits = self.mtl_adv_classifiers[task_id-1](sequence_output[:,0,:], rev_grad=True)
                attr_probs = torch.nn.Softmax(-1)(attr_logits)
                adv_loss = self.mtl_adv_classifiers[task_id-1].compute_loss(attr_pred=attr_logits, attr_gt=attr,
                                                       label_gt=labels, label_stats=reg_args.label_stats)
                loss += adv_loss * reg_args.adv_strength

            outputs = (loss,) + outputs

        outputs = outputs + ({'adv_loss': adv_loss, 'attr_probs': attr_probs}, )  # extra returns will be updated in reversed order
